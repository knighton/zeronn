...
void conv4d_init(layer_t* layer, int x_channels, int y_channels, int face,
    ...
    f->weight = uniform(fx_from_fp(-0.05), fx_from_fp(0.05), x_channels,
                        y_channels, f->face.time, f->face.depth,
                        f->face.height, f->face.width);
    f->dweight = zeros(x_channels, y_channels, f->face.time, f->face.depth,
                       f->face.height, f->face.width);
    f->bias = uniform(fx_from_fp(-0.05), fx_from_fp(0.05), y_channels);
...
layer_t* conv4d(int x_channels, int y_channels, int face, int stride,
                 int pad) {
    layer_t* layer = (layer_t*)malloc(sizeof(conv4d_t));
    conv4d_init(layer, x_channels, y_channels, face, stride, pad);
    return layer;
}
...
tensor_t* conv4d_forward(layer_t* layer, tensor_t* x, bool is_t) {
    ...
    for (int i = 0; i < y->size; ++i) {
        y->data[i] = fx_div(y->data[i], FX_ONE);
    }
...
tensor_t* conv4d_backward(layer_t* layer, tensor_t* dy) {
    ...
    tensor_t* dx = zeros(batch_size, f->x_channels, x_time, x_depth,
                         x_height, x_width);
    tensor_t* dweight = zeros(f->x_channels, f->y_channels, f->face.time,
        ...
        for (int y_c = 0; y_c < f->y_channels; ++y_c) {
            ...
            for (int x_c = 0; x_c < f->x_channels; ++x_c) {
                ...
                int w_idx = ((((x_c * f->y_channels + y_c) * f->face.time +
                    face_t) * f->face.depth + face_d) * f->face.height +
                    face_h) * f->face.width + face_w;
                dweight->data[w_idx] += dy_val * f->x->data[x_idx];
    ...
    for (int i = 0; i < dweight->size; ++i) {
        f->dweight->data[i] += fx_div(dweight->data[i], FX_ONE);
    }
    for (int i = 0; i < dx->size; ++i) {
        dx->data[i] = fx_div(dx->data[i], FX_ONE);
    }
    tensor_free(dweight);
    free(dweight);
...
void conv4d_update_step(layer_t* layer, fx_t lr) {
    conv4d_t* f = (conv4d_t*)layer;
