...
void linear_init(layer_t* layer, int x_dim, int y_dim) {
    ...
    f->x_dim = x_dim;
    f->y_dim = y_dim;
    f->weight = uniform(-128, 128, x_dim, y_dim);
    f->dweight = zeros(x_dim, y_dim);
    f->bias = uniform(-128, 128, y_dim);
    f->dbias = zeros(y_dim);
    f->x = empty();
}
...
tensor_t* linear_forward(layer_t* layer, tensor_t* x, bool is_t) {
    ...
    for (int n = 0; n < count; ++n) {
        for (int yd = 0; yd < f->y_dim; ++yd) {
            ...
            y->data[y_idx] = fx_div(y_val, FX_ONE) + f->bias->data[yd];
        }
    }
    return y;
}
...
void linear_update_step(layer_t* layer, fx_t lr) {
    linear_t* f = (linear_t*)layer;
