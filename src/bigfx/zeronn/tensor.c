...
void tensor_init_common(tensor_t* x, int num_args, va_list* args) {
    ...
    x->data = (bigfx_t*)malloc(x->size * sizeof(bigfx_t));
}
...
void tensor_uniform_log2(tensor_t* x, bigfx_t low, uint8_t log2_range) {
    for (int i = 0; i < x->size; ++i) {
        x->data[i] = prng_randint_log2(low, log2_range);
    }
}
...
tensor_t* pp_uniform_log2(bigfx_t low, uint8_t log2_range, int num_args, ...) {
    ...
    tensor_init_common(x, num_args, &args);
    va_end(args);
    tensor_uniform_log2(x, low, log2_range);
    return x;
}
...
void tensor_set(tensor_t* x, tensor_t* a) {
    if (x->size != a->size) {
        ...
        x->data = (bigfx_t*)malloc(a->size * sizeof(bigfx_t));
    }
    x->ndim = a->ndim;
...
void tensor_update_step(tensor_t* x, tensor_t* dx, bigfx_t lr) {
    ...
    for (int i = 0; i < x->size; ++i) {
        d = bigint_mul(dx->data[i], lr);
        d = bigint_div_log2(d, FX_BITS);
        x->data[i] = bigfx_sub(x->data[i], d);
    }
}
