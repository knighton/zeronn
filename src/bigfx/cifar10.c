...
#define DROP bigfx_from_float(1.0 / 2)
#define MOM bigfx_from_float(7.0 / 8)
#define EPS bigfx_from_float(1.0 / 128)
...
void sample(dataset_t* data, tensor_t* x, int* y) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = 0; j < d; ++j) {
            uint8_t b = data->x[s * d + j];
            x->data[i * d + j] = bigfx_div_log2(bigfx_from_int(b), 8);
        }
        y[i] = data->y[s];
    }
}
...
int main(void) {
    ...
    int batch_size = 10;
    int model_dim = 32;
    bigfx_t lr = bigfx_from_float(1.0 / 256);
