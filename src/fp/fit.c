...
void get_batch(int out_dim, float signal, tensor_t* x, int* y) {
...
int get_correct(tensor_t* y_pred, int* y_gold) {
    ...
    for (int i = 0; i < batch_size; ++i) {
        int j_max = 0;
        float y_max = y_pred->data[i * dim];
        for (int j = 1; j < dim; ++j) {
            float y = y_pred->data[i * dim + j];
            if (y_max < y) {
                j_max = j;
                y_max = y;
            }
...
int main(void) {
    ...
    for (int i = 0; i < num_epochs; ++i) {
        ...
        float t = 100.0 * ta / (rounds_per_epoch * train_per_round *
                                batch_size);
        float v = 100.0 * va / (rounds_per_epoch * eval_per_round *
                                batch_size);
        printf("%7d %7.3f %7.3f\n", i, t, v);
    }
